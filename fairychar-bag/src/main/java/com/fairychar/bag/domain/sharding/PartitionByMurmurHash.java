package com.fairychar.bag.domain.sharding;
...
//			}
    ...
    private void generateBucketMap() {
        this.hash = Hashing.murmur3_32(this.seed);//计算一致性哈希的对象
        for (int i = 0; i < this.count; i++) {//构造一致性哈希环，用TreeMap表示
            StringBuilder hashName = new StringBuilder("SHARD-").append(i);
            for (int n = 0, shard = this.virtualBucketTimes * this.getWeight(i); n < shard; n++) {
                this.bucketMap.put(this.hash.hashUnencodedChars(hashName.append("-NODE-").append(n)).asInt(), i);
            }
        }
        this.weightMap = null;
    }
    ...
    private int getWeight(int bucket) {
        Integer w = this.weightMap.get(bucket);
        if (w == null) {
            w = DEFAULT_WEIGHT;
        }
        return w;
    ...
    public void setWeightMapFile(String weightMapPath) throws IOException {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(this.getClass().getClassLoader().getResourceAsStream(weightMapPath), DEFAULT_CHARSET))) {
            ...
            for (Map.Entry entry : props.entrySet()) {
                ...
                this.weightMap.put(Integer.parseInt(entry.getKey().toString()), weight > 0 ? weight : 1);
            }
        }
    }
    ...
    public Integer calculate(String columnValue) {
        SortedMap<Integer, Integer> tail = this.bucketMap.tailMap(this.hash.hashUnencodedChars(columnValue).asInt());
        if (tail.isEmpty()) {
